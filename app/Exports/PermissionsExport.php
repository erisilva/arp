...
class PermissionsExport implements FromQuery, WithHeadings
{
    ...
    */
    ...
    {
        $query = Permission::query()->select('name', 'description')->orderBy('id', 'asc');

        // Verifica se o modelo Permission tem o escopo filter ou aplica filtro manualmente
        if ($this->filter) {
            if (method_exists(Permission::class, 'scopeFilter')) {
